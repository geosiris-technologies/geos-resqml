...
// MyHdfProxyMPI::MyHdfProxyMPI(const std::string & packageDirAbsolutePath, const std::string & externalFilePath, COMMON_NS::DataObjectRepository::openingMode hdfPermissionAccess) :
...
void MyHdfProxyMPI::writeUuidAttribute()
{
...
void MyHdfProxyMPI::readArrayNdOfValues(const std::string & datasetName, void* values, hid_t datatype)
{
	if (!isOpened()) {
		open();
	}
...
void MyHdfProxyMPI::readArrayNdOfValues(
...
void MyHdfProxyMPI::selectArrayNdOfValues(
	const std::string & datasetName,
	uint64_t const * blockCountPerDimension,
	uint64_t const * offsetInEachDimension,
	uint64_t const * strideInEachDimension,
...
COMMON_NS::AbstractObject::numericalDatatypeEnum MyHdfProxyMPI::getNumericalDatatype(const std::string & datasetName)
{
	if (!isOpened()) {
		open();
	}
...
int MyHdfProxyMPI::getHdfDatatypeClassInDataset(const std::string & datasetName)
{
	if (!isOpened()) {
		open();
	}
...
void MyHdfProxyMPI::writeItemizedListOfList(const string & groupName,
	const string & name,
...
std::vector<uint32_t> MyHdfProxyMPI::getElementCountPerDimension(const std::string & datasetName)
{
	if (!isOpened()) {
		open();
	}
...
void MyHdfProxyMPI::writeArrayNd(const std::string & groupName,
...
void MyHdfProxyMPI::writeArrayNdSlab(
...
void MyHdfProxyMPI::readArrayNdOfDoubleValues(const std::string & datasetName, double* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_DOUBLE);
}

void MyHdfProxyMPI::readArrayNdOfDoubleValues(
...
void MyHdfProxyMPI::readArrayNdOfFloatValues(const std::string & datasetName, float* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_FLOAT);
}

void MyHdfProxyMPI::readArrayNdOfFloatValues(
...
void MyHdfProxyMPI::readArrayNdOfInt64Values(const std::string & datasetName, int64_t* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_INT64);
}

void MyHdfProxyMPI::readArrayNdOfInt64Values(
...
void MyHdfProxyMPI::readArrayNdOfUInt64Values(const std::string & datasetName, uint64_t* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_UINT64);
}

void MyHdfProxyMPI::readArrayNdOfIntValues(const std::string & datasetName, int* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_INT);
}

void MyHdfProxyMPI::readArrayNdOfIntValues(
...
void MyHdfProxyMPI::readArrayNdOfUIntValues(const std::string & datasetName, unsigned int* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_UINT);
}

 void MyHdfProxyMPI::readArrayNdOfShortValues(const std::string & datasetName, short* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_SHORT);
}

void MyHdfProxyMPI::readArrayNdOfUShortValues(const std::string & datasetName, unsigned short* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_USHORT);
}

void MyHdfProxyMPI::readArrayNdOfInt8Values(const std::string & datasetName, int8_t* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_CHAR);
}

void MyHdfProxyMPI::readArrayNdOfUInt8Values(const std::string & datasetName, uint8_t* values)
{
	readArrayNdOfValues(datasetName, values, H5T_NATIVE_UCHAR);
}

hid_t MyHdfProxyMPI::openOrCreateGroup(const string & groupName)
{
...
void MyHdfProxyMPI::writeGroupAttributes(const std::string & groupName,
...
void MyHdfProxyMPI::writeGroupAttribute(const std::string & groupName,
...
void MyHdfProxyMPI::writeDatasetAttributes(const std::string & datasetName,
	const std::vector<std::string> & attributeNames,
	const std::vector<std::string> & values)
{
	if (attributeNames.size() != values.size()) {
...
void MyHdfProxyMPI::writeDatasetAttribute(const std::string & datasetName,
	const std::string & attributeName,
	const std::vector<std::string> & values)
{
	hid_t dataset = H5Dopen(hdfFile, datasetName.c_str(), H5P_DEFAULT);
...
std::string MyHdfProxyMPI::readStringAttribute(const std::string & obj_name,
...
vector<string> MyHdfProxyMPI::readStringArrayAttribute(const std::string & obj_name,
	const std::string & attr_name) const
{
	if (!isOpened()) {
		throw invalid_argument("The HDF5 file is not opened");
...
double MyHdfProxyMPI::readDoubleAttribute(const std::string & obj_name,
	const std::string & attr_name) const
{
	if (!isOpened()) {
		throw invalid_argument("The HDF5 file is not opened");
...
int64_t MyHdfProxyMPI::readInt64Attribute(const std::string & obj_name,
	const std::string & attr_name) const
{
	if (!isOpened()) {
...
bool MyHdfProxyMPI::exist(const std::string & absolutePathInHdfFile) const
{
	return H5Oexists_by_name(hdfFile, absolutePathInHdfFile.c_str(), H5P_DEFAULT) > 0;
}

bool MyHdfProxyMPI::isCompressed(const std::string & datasetName)
{
	if (!isOpened()) {
		open();
	}
...
std::vector<uint32_t> MyHdfProxyMPI::getElementCountPerChunkDimension(const std::string & datasetName)
{
	if (!isOpened()) {
		open();
	}
